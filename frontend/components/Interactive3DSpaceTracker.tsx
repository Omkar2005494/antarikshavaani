...
import { X, Globe, Moon, Play, Pause, Satellite, Compass, Target, RotateCcw, MessageSquare, ChevronDown, ChevronUp, Radio, Activity } from "lucide-react";
...
<span className="px-2 py-0.5 rounded-full text-[9px] font-mono font-bold bg-cyan-500/10 text-cyan-400 border border-cyan-500/30">
      ...
      {/* Sleek, Glassmorphic Aerospace Telemetry HUD (Bottom-Left Dock) */}
      <div className="absolute bottom-24 left-4 sm:left-6 z-20 pointer-events-auto max-w-sm w-full transition-all duration-300">
        {!hudExpanded ? (
          ...
          >
            ...
            <span>
              {mode === "earth" ? selectedSatellite?.name : selectedSite?.name.split("(")[0]} Telemetry
            </span>
            <ChevronUp className="w-3.5 h-3.5 text-slate-400 group-hover:translate-y-[-1px] transition-transform" />
          </button>
        ) : (
          /* Expanded Aerospace Glassmorphic Instrument Card */
          <div className="p-4 rounded-2xl bg-slate-950/60 backdrop-blur-3xl border-l-2 border-l-cyan-400 border border-white/10 shadow-2xl shadow-cyan-950/40 text-slate-200 space-y-3">
            ...
            {mode === "earth" && selectedSatellite && (
              <div className="space-y-3">
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      ...
                      <p className="text-[10px] font-mono text-slate-400">Spacecraft Bus Ground-Truth</p>
                    </div>
                  </div>
                ...
                {/* 4-Cell Aerospace KPI Grid */}
                <div className="grid grid-cols-2 gap-2 text-xs font-mono">
                  <div className="p-2 rounded-xl bg-white/5 border border-white/5">
                    <span className="text-[9px] uppercase tracking-wider text-slate-400 block mb-0.5">Orbit Class</span>
                  ...
                  <div className="p-2 rounded-xl bg-white/5 border border-white/5">
                    <span className="text-[9px] uppercase tracking-wider text-slate-400 block mb-0.5">Subsystem Health</span>
              ...
              <div className="space-y-3">
                ...
                <div className="p-2.5 rounded-xl bg-white/5 border border-white/5 text-xs font-mono space-y-1.5">
                  <div className="flex justify-between text-[11px]">
                    <span className="text-slate-400">Lunar Coordinates:</span>
