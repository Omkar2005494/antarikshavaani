...
const LUCKY_QUERIES = [
  "What is the current position of the Mangalyaan mission?",
  "Explain the GSLV Mk III launch vehicle specifications.",
  "Tell me about the Astrosat space telescope discoveries.",
  "What are the future ISRO missions planned for 2024?",
  "Details on the Gaganyaan human spaceflight program.",
  "How does NAVIC compare to GPS?",
  "What is the NISAR mission with NASA?",
  "Tell me a fun fact about ISRO's history."
];
...
const STARTER_PROMPTS = [
  ...
  },
  {
    ...
    subtitle: "Let AntarikshaVaani surprise you with a random deep-space query",
    query: "LUCKY",
    tier: "✨ Random"
  }
];
        ...
        <div className="flex-1 overflow-y-auto px-3 py-2 space-y-1 custom-scrollbar text-xs">
          ...
          {STARTER_PROMPTS.map((p, i) => (
            <button
              key={i}
              onClick={() => { 
                const q = p.query === "LUCKY" ? LUCKY_QUERIES[Math.floor(Math.random() * LUCKY_QUERIES.length)] : p.query;
                handleSend(q); 
        ...
        <div className="flex-1 overflow-y-auto px-4 sm:px-8 lg:px-12 py-6 pb-44 custom-scrollbar">
          {messages.length === 0 ? (
            ...
            <div className="min-h-[65vh] flex flex-col items-center justify-center text-center max-w-5xl mx-auto my-auto space-y-8">
              ...
              {/* 3x2 Minimalist Suggestion Grid */}
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 w-full text-left">
                {STARTER_PROMPTS.map((starter, i) => (
                  <button
                    key={i}
                    onClick={() => {
                      if (starter.query === "LUCKY") {
                        const randomQuery = LUCKY_QUERIES[Math.floor(Math.random() * LUCKY_QUERIES.length)];
                  ...
                  >
                    <div>
                      <div className="flex items-center justify-between mb-1">
                        ...
                        {starter.tier && (
                          <span className="text-[9px] px-1.5 py-0.5 rounded-full bg-slate-800/80 border border-slate-700/50 text-slate-300 ml-2 whitespace-nowrap">
                            {starter.tier}
                          </span>
                        )}
                      </div>
                              ...
                              <div className="pt-3 border-t border-slate-800/80 space-y-2">
                                ...
                                <div className="space-y-1">
                                  {msg.citations.map((cite, idx) => (
                                    <div key={idx} className="text-[11px] text-slate-400 flex items-start gap-1.5 group">
                                      <span className="text-cyan-500 font-mono text-[9px] mt-0.5">[{idx + 1}]</span>
                                      <a href="#" className="hover:text-cyan-400 hover:underline transition-colors leading-relaxed">
                                        {cite}
                                      </a>
                                    </div>
                                  ))}
